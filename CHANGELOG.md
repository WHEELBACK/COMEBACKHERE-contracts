...
- Atomic emergency pause coordination across the protocol (#73). `emergency_pause_all` fans `pause` out over a pinned, ordered set of protocol contracts (invoice, treasury, compliance) in a single call, and it is **all-or-nothing**: the first target that does not confirm the pause aborts the entire call with `PauseTargetFailed`, and because returning an error unwinds the top-level transaction, the targets already paused are rolled back with it. The failure this closes is the one in the issue — a sweep that carries on past a refusal leaves the protocol split, with some contracts stopped and others still moving money, and no way for an operator to tell which. The guarantee is structural rather than best-effort: failures are never collected (deliberately unlike the per-settlement skip in `execute_with_compliance_batch`), and `EmergencyPausedAt` is written only after the last target confirms, so its presence means the whole protocol is stopped and its absence means this contract paused none of it. `resume_all` mirrors the rule in reverse order — last paused, first resumed — so recovery runs from the outside of the pipeline inwards, and a failed resume rolls back the targets already resumed rather than leaving the protocol half-open. `initialize_emergency_pause` pins the pause admin and the target set and validates it up front (non-empty, at most `MAX_PAUSE_TARGETS`, no duplicates, never the orchestrator itself), so a misconfiguration is rejected before a byte is stored rather than discovered halfway through an incident; the set can be repointed only while no incident is open, since a new set would not describe the contracts that are actually paused. Coordination failures are reported through a new `WorkflowError` rather than the borrowed `TreasuryError`, which is borrowed from `multisig` only because the settlement path already depended on it. The orchestrator is granted exactly one power over the rest of the protocol — the ability to stop it — and holds no other authority: the client trait declares `pause`/`unpause` and nothing else.
